...
We write the adjunction as $F \dashv G$. Packing things up, the adjunction is a quadruple $(F, G, \leftrightarrow_{bij}, bijective(bij))$:
- $F$ and $G$ are functors
- $\leftrightarrow_{bij}$ is a bijection between hom-sets(function spaces)
- $bijective(bij)$ is the proof that $\leftrightarrow_{bij}$ is indeed a bijection.

Note: Adjunctions are also called Galois connections in fields like order theory, abstract algebra, etc.


We shall give some examples of adjunctions, with the focus on the construction of the bijection $\leftrightarrow_{bij}$ and proof of bijectiveness.


Example : **Free-forgetful adjunction** between sets and free groups
Let $F: Set \to Group$ and $U: Group \to Set$ be the free functor and forgetful functor respectively, as defined below:
- Free functor $F$: $F(S)$ is the free group generated by a set $S$. For a function $f: S \to T$ between sets, $F(f): F(S) \to F(T)$ is the induced group homomorphism.
- Forgetful functor $U$: Given a group $G$, $U(G)$ is the underlying set of $G$. 


Let $bij$ be the bijection in $Hom_{Group}(F(S), G) \leftrightarrow_{bij} Hom_{Set}(S, U(G))$, steps:
- $h: F(S) \to G$, $h \in Hom_{Group}(F(S), G)$ is a group homomorphism
- given $h$, we shall construct a function $h': S \to U(G)$, $h' \in Hom_{Set}(S, U(G))$ as a function of sets.
- Let $a \in S$, then $F(a) \in F(S)$, so now we define $h'(a) = [h(F(a))] \in U(G)$.
- so $bij(h) = h'= (a: S \mapsto U(h(F(a))))$.

proof of injectiveness:
- suppose $bij(h_1) = bij(h_2)$, we want to show $h_1 = h_2$. where $h_1, h_2 \in Hom_{Group}(F(S), G)$.
- that means if functions of sets are equal, then the group homomorphisms are equal, which is true since group homomorphisms are determined by their values on generators.


Example: **partial order(poset) adjunction**
Let $(P, \leq_P)$ and $(Q, \leq_Q)$ be two posets as categories(objects are elements, morphisms only exist when $a \leq b$), so a functor is just a monotone function.

Let $F: P \to Q$ and $G: Q \to P$ be two functors(monotone functions). We say $F$ is left adjoint to $G$ if there's a bijection:
$$
Hom_Q(F(p), q) \leftrightarrow_{bij} Hom_P(p, G(q))
$$
This just translates to:
$$
F(p) \leq_Q q \iff p \leq G(q)
$$
Where the bijection is just the logical equivalence if and only if($\iff$).
